...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $prd= $_POST['id'];
    ...
    if (isset($_FILES['imagen']['name'])) {
        ...
        $carpeta_destino = '../../views/admin/html/img_producto/';
        $ruta_imagen = $carpeta_destino . $nombre_archivo;

        if (move_uploaded_file($_FILES['imagen']['tmp_name'], $ruta_imagen)) {
            // Inserta la ruta de la imagen en la base de datos
            $insertQry = "UPDATE `img_productos`SET`nombre_imagen`='$nombre_archivo' WHERE img_productos.id_producto=$prd";
    ...
    header("Location:/var/www/geekhaven/src/views/admin/html/editImagen.php?mensaje=success&id=$prd&img=$nombre_archivo");
} 
