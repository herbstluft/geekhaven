...
foreach($Productos as  $res){
    ...
    if (!empty($validacionQry)){
        ...
        <hr>
        <center>
        <a href="http://'.$HOST.'/var/www/geekhaven/" class="btn btn-primary pt-2 pb-2 fs-5">Click aqui para volver</a>
        </center>';
        header('refresh: 1; url=http://'.$HOST.'/var/www/geekhaven/src/scripts/cart/ticketCart.php?id_orden='.$id_orden);
    }
